    ...
    <section>
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="relative bg-purple-600 py-10 px-8 md:py-16 md:px-12" data-aos="fade-up">
          ...
          <div className="relative flex flex-col lg:flex-row justify-between items-center">
            ...
            {/* CTA form */}
            <form id="subscribeForm" className="w-full lg:w-1/2">
        <div className="flex flex-col sm:flex-row justify-center max-w-xs mx-auto sm:max-w-md lg:max-w-none">
          <input type="email" name="email" className="w-full appearance-none bg-purple-700 border border-purple-500 focus:border-purple-300 rounded-sm px-4 py-3 mb-2 sm:mb-0 sm:mr-2 text-white placeholder-purple-400" placeholder="Your best email…" aria-label="Your best email…" />
          <button type="submit" className="btn text-purple-600 bg-purple-100 hover:bg-white shadow">Subscribe</button>
        </div>
        <div id="formMessage" className="mt-2 text-center"></div>
      </form>
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        document.getElementById('subscribeForm').addEventListener('submit', async function(event) {
          ...
          try {
            const response = await fetch('https://www.backend.repodex.ai/api/contact', {
              method: 'POST',
              headers: {
            ...
            formMessage.textContent = 'There was a problem processing your request. Please try again later.';
            formMessage.style.color = 'red';
          }
        });        
      `}} />
